...
# modified for hackeriot-workshop-2024 : https://github.com/agentzex/hackeriot-workshop-2024
...
def parse_args(description):
    ...
    parser.add_argument('-S', '--ssh_port',
                        help='Port number to bind SSH server to, on the host', type=int, required=True)

    parser.add_argument('-P', '--server_port_range',
                        help="Range of ports or single port to bind on the host, that will be used for agents servers "
                             "Example: 6660 or 6660-6669",
                        required=True)


    args = parser.parse_args()

    # check that -P and -S are valid ports
    validate_port(args.ssh_port)
    ports_range = split_ports(args.server_port_range)
    # if ports are valid assign them to args
    args.server_port_range = ports_range
...
def split_ports(ports_range):
    ports = []

    if '-' in ports_range:
        # Split the input into start and end of the range
        start_port, end_port = ports_range.split('-')
        if int(start_port) > int(end_port):
            print("Start of port range can't be bigger than end of port range")
            exit(-1)
        # Convert to integers and create the range
        ports = list(range(int(start_port), int(end_port) + 1))
    else:
        # Single port, convert it to an integer and put in a list
        ports = [int(ports_range)]

    list(map(validate_port, ports))  # executing validate_port for each port given
    return ports
...
if __name__ == "__main__":
    ...
    # add each input port as a port to bind on the host
    for p in args.server_port_range:
        envs += ["-p", f"{p}:{p}"]

    port_ssh = str(args.ssh_port)
    envs += ["-p", f"{port_ssh}:22"]
